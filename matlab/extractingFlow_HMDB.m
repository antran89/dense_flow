...
function extractingFlow_HMDB(index, device_id, type)
...
if type ==0
    path2 = '/media/data/hmdb_flow/hmdb_flow_img_farn_gpu_s2/';
elseif type ==1
    path2 = '/media/data/hmdb_flow/hmdb_flow_img_tvl1_gpu_s2/';
else
    path2 = '/media/data/hmdb_flow/hmdb_flow_img_brox_gpu_s2/';
end
...
for i = index
    ...
    for j = 1:length(filelist)
        ...
        cmd = sprintf('../src-build/denseFlow_gpu -f=''%s'' -x=''%s'' -y=''%s'' -b=20 -t=%d -d=%d -s=%d',...
            file1,file2,file3,type,device_id,2);
        system(cmd);
    end
end
end